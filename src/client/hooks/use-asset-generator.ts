...
import { ASSET_DEFS, Platform } from '../lib/asset-definitions';
...
export interface AssetGeneratorState {
  ...
  bg: BgConfig;
  cornerRadius: number;
  bgRemoveThreshold: number;          // 0–100 euclidean distance
...
export function useAssetGenerator() {
  ...
  const [state, setState] = useState<AssetGeneratorState>({
    ...
    cornerRadius: 22,
    bgRemoveThreshold: 40,
    monochromeColor: 'white',
  });

  const setSourceFile = useCallback((file: File) => {
    setState((s) => {
      if (s.sourcePreview) URL.revokeObjectURL(s.sourcePreview);
      return {
        ...s,
        sourceFile: file,
  ...
  const setBgRemoveThreshold = useCallback((v: number) =>
    setState((s) => ({ ...s, bgRemoveThreshold: v })), []);

  const setMonochromeColor = useCallback((v: 'white' | 'black') =>
    setState((s) => ({ ...s, monochromeColor: v })), []);
  ...
  const generate = useCallback(async () => {
    ...
    try {
      const opts: GenerateOptions = {
        ...
        bgRemoveThreshold: state.bgRemoveThreshold,
  ...
  }, [state.sourceFile, state.foregroundScale, state.bg, state.cornerRadius, state.bgRemoveThreshold, state.monochromeColor, state.enabledPlatforms, toast]);
  ...
  const downloadAllZip = useCallback(async () => {
    ...
    zip.file('app.json.example', buildExpoAppJson(state.bg.color));
